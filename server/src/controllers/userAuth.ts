...
interface FrontendEmployeeData {
  employeeFirstName: string;
  employeeLastName: string;
  employeeEmail: string;
  employeePassword: string;
...
export async function handleUserRegister(
  ...
  try {
    ...
    if (await createEmployeeLeaveTable(savedEmployee.id)) {
...
async function createEmployeeLeaveTable(employeeId: number): Promise<boolean> {
  try {
    const AppDataSource = await getConnection();
    ...
    await leaveRepository.save(newLeaveRecord);
    return true;
  } catch (error) {
...
export async function handleUserSignin(
  ...
  const {
    employeeEmail,
    employeePassword,
  }: { employeeEmail: string; employeePassword: string } = req.body;
  ...
  try {
    ...
    // Generate JWT token
    const token = await handleGenerateToken(employee);
...
export async function handleGetUserDetails(
  ...
  try {
    ...
    if (!employee) {
      ...
      const leaveDetails = employee.leaves?.[0] || {};
      const { leaves, ...employeeData } = employee;
...
export async function handleCurrEmployeePersonalData(
  req: Request,
  res: Response
): Promise<void> {
  const {
  ...
  try {
    ...
    const response = {
      ...updatedEmployee,
      ...updatedEmployee.leaves?.[0], // Flatten leaves (if applicable)
    };
...
export async function handleSendReferral(
  req: AuthenticatedRequest,
  res: Response
): Promise<void> {
  try {
...
export async function getAssignedCandidatesForManager(
  ...
  try {
    ...
    const assignmentRepository =
      AppDataSource.getRepository(ReferralAssignment);
...
export async function handleTodoCreation(
  ...
  try {
    ...
    const AppDataSource = await getConnection();
    const todoRepository = AppDataSource.getRepository(TodoTable);
    const employeeRepository = AppDataSource.getRepository(EmployeeDetails);
...
/** Get todo items */
export async function handleGetTodoItem(
  req: AuthenticatedRequest,
  res: Response
): Promise<void> {
  try {
...
export async function handleBlobCreation(
  ...
  try {
    const AppDataSource = await getConnection();
    const blobRepository = AppDataSource.getRepository(BlogTable);
    ...
    const savedBlob = await blobRepository.save(newBlob);
...
export async function getAllBlobs(req: Request, res: Response): Promise<void> {
  try {
    const AppDataSource = await getConnection();
    const blogRepository = AppDataSource.getRepository(BlogTable);

    // Fetch blobs with employee details, ordered by created_at
    const blobs = await blogRepository.find({
      relations: ["employee"],
      order: {
