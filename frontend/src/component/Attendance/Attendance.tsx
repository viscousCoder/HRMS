...
const Attendance = ({ isClicked }: propsType) => {
  const { data, loading } = useSelector<RootState>((state) => state.calender);
  ...
  useEffect(() => {
    console.log(isClicked, "hello", newData);
    if (isClicked) {
      let data = true;
...
// // Set initial events to use "2025/02/24" explicitly
// const initialEvents = [
//   {
//     id: 1,
//     title: "Team Meeting",
//     start: dayjs("2025/02/24"), // Use YYYY/MM/DD format
//     end: dayjs("2025/02/24").add(1, "hour"),
//     type: 1,
//     description: "Weekly team sync",
//   },
//   {
...
//     ?.map((item) => ({
//       ...item,
//       start: dayjs(item.start), // Ensure parsing works with incoming data
//       end: dayjs(item.end),
//     }));

//   // Set initial currentDate to "2025/02/24"
//   const [currentDate, setCurrentDate] = useState(dayjs("2025/02/24"));
//   const [events, setEvents] = useState(initialEvents);
//   const [selectedDate, setSelectedDate] = useState(null);
//   const [openEventForm, setOpenEventForm] = useState(false);
//   const [selectedEvent, setSelectedEvent] = useState(null);
...
//   const [eventForm, setEventForm] = useState({
//     title: "",
//     start: dayjs("2025/02/24"), // Default to 2025/02/24
//     end: dayjs("2025/02/24").add(1, "hour"),
//     type: 1,
//     description: "",
//     isEveryone: false,
...
//       for (let i = 0; i < newData?.length; i++) {
//         if (
//           newData[i].start.format("YYYY/MM/DD") ===
...
//           id: Date.now(),
//           title: "Present",
//           start: dayjs("2025/02/24"), // Use today's date or "2025/02/24" as needed
//           end: dayjs("2025/02/24").add(1, "hour"),
//           type: 5,
//           description: "Employee is present",
//           isEveryone: false,
...
//   const handleEventSubmit = (all: boolean) => {
//     if (eventForm.start.isAfter(eventForm.end)) {
//       setSnackbar({
//         open: true,
//         message: "Start time must be before end time",
...
//     if (eventForm.title !== "") {
//       const eventData = { ...eventForm, isEveryone: all };
//       if (selectedEvent) {
//         setEvents(
//           events?.map((event) =>
//             event.id === selectedEvent.id
...
//           severity: "success",
//         });
//         dispatch(handleSendCalenderData(eventData)).then(() => {
//           dispatch(handleGetCalenderData());
//         });
//       }
//     }
...
//     setEventForm({
//       title: "",
//       start: dayjs("2025/02/24"), // Reset to 2025/02/24
//       end: dayjs("2025/02/24").add(1, "hour"),
//       type: 1,
//       description: "",
//       isEveryone: false,
...
//           key={day.toString()}
//           onClick={() => handleDateClick(day)}
//           isToday={day.isSame(dayjs("2025/02/24"), "day")} // Compare with 2025/02/24
//           days={day.format("D")}
//           hasEvents={dayEvents.length > 0}
//         >
//           <Typography variant="body2">{day.format("D")}</Typography>
...
//             startIcon={<FaCalendarPlus />}
//             variant="contained"
//             onClick={() => handleDateClick(dayjs("2025/02/24"))} // Default to 2025/02/24
//           >
//             Add Event
//           </Button>
//         </Box>
